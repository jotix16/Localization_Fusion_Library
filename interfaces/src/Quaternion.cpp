...
std::size_t Quaternion::serialize(void *pOutput) const
{
    if (!pOutput)
        return 0;
...
std::size_t Quaternion::deserialize(const void *pInput)
{
    if (!pInput)
        return 0;
    ...
    buf += 8;
    w = *reinterpret_cast<const double*>(buf);
    buf += 8;
...
Quaternion::Quaternion(const geometry_msgs::msg::Quaternion &other)
{
...
std::size_t Quaternion::getSerializedSize() const
{
	std::size_t _size_ = 0;
...
geometry_msgs::msg::Quaternion& Quaternion::operator=(const geometry_msgs::msg::Quaternion &other)
{
    if (this != &other)
    {
        x = other.x;
